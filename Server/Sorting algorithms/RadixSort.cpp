...
#include "RadixSort.h"
size_t RadixSort ::getMax(vector<string> &arr, int n) {
...
void RadixSort ::countSort(vector<string> &a, int size, size_t k) {
...
void RadixSort ::radixSort(vector<string> &b, int r) {
...
void RadixSort ::radixSort(vector<string> &arr) {
    for (int i = 0 ; i < arr.size(); i++) {
        arr[i] = mayusConvert::mayus(arr[i]);
    }
